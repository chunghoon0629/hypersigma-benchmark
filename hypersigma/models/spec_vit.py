"""
Spectral Vision Transformer (SpecViT) for HyperSIGMA

Processes spectral dimension using a ViT backbone with adaptive spectral tokenization.
"""
...
def deform_inputs_func(x, num_tokens):
    """Generate deformable inputs for spectral encoder."""
    spatial_shapes = torch.as_tensor(
        [int(math.sqrt(num_tokens)), int(math.sqrt(num_tokens))],
        dtype=torch.long, device=x.device
    )
    reference_points = get_reference_points(
        [int(math.sqrt(num_tokens)), int(math.sqrt(num_tokens))], x.device
    )
...
class SampleAttention(nn.Module):
    """Deformable sampling-based attention for spectral processing."""
...
class PatchEmbed(nn.Module):
    """Spectral patch embedding (flatten spatial dimension)."""
...
class SpectralVisionTransformer(nn.Module):
    ...
    Processes spectral dimension using adaptive tokenization and ViT backbone.
    """

    def __init__(self, NUM_TOKENS=None, img_size=224, in_chans=3, embed_dim=768,
                 ...
                 norm_layer=None, init_values=None, use_checkpoint=False,
        ...
        # Cross-attention modules for multi-scale feature extraction
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            print(f"Loaded SpecViT weights: {msg}")
